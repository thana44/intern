...
function AllReview() {
    const columns = [
        ...
        {
            title: 'ชื่อสถานที่รีวิว (ภาษาไทย)',
            dataIndex: 'thaiName',
            key: 'thaiName'
        },
        {
            title: 'ชื่อสถานที่รีวิว (อังกฤษ)',
            dataIndex: 'engName',
            key: 'engName'
        },
        {
    ...
    const handleTableChange = (pagination) => {
        getAllReviewPaginate(pagination.current, pagination.pageSize)
    }

    const getAllReviewPaginate = async (page = 1, pageSize = 5) => {
        try {
            await axios.get(`${import.meta.env.VITE_BACKEND_URL}/review-api/get-paginate-all-review?page=${page}&pageSize=${pageSize}&search=${search}`, { withCredentials: true })
                .then((res) => {
    ...
    useEffect(() => {
        getAllReviewPaginate(1, pagination.pageSize)
    }, [search])
    ...
    const deleteReview = async () => {
        setCLoading(true)
        try {
            await axios.post(`${import.meta.env.VITE_BACKEND_URL}/review-api/delete-review-from-all`, { reviewId: delId, placeId, userId, detail }, { withCredentials: true })
                .then((res) => {
            ...
            <div className='bg-gray-200 w-full'>
                ...
                <div className='mt-5  flex justify-center'>
                    <div className=' px-3 pb-10 w-full lg:w-[80%]'>
                        <div className='text-md font-bold mb-5'>รีวิวทั้งหมด</div>
                        <div className=' mb-5 flex gap-3 items-center justify-between'>
                            <div className='w-full md:w-[400px]'>
                                <Input suffix={<SearchOutlined />} onChange={(e) => setSearch(e.target.value)} size='large' placeholder="ค้นหาชื่อผู้รีวิว/ชื่อสถานที่รีวิว" />
                            </div>
